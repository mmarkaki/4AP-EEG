...
%
% if fixed_params=[0, 1 , NaN , NaN] normalization here
...
% fsigm = @(param,xval)
%param(1)+exp.^((param(3)-xval)*param(4)))
%rmin + exp^(x50-xval)*slope)
% param=[0 1 5 1];  % "min", "x50", "slope"
% x=0:0.1:10;
...
if sum(y==quantile(y,0.5))==0%1
    temp=x(y==quantile(y(2:end),0.5));    
else
    temp=x(y==quantile(y,0.5));%-2.2833 at 2279
end
...
%p(1)=min; p(2)=max-min; p(3)=x50;
%p(4)=slope como em Y=Bottom + (Top-Bottom)/(1+10^((LogEC50-X)*HillSlope))
...
for i=1:4;
    ...
    if i==2;
        if isnan(fixed_params(1))            
            f_str=[f_str '-param(1) )./ (   1 + epsi.^( (']; 
        else
            f_str=[f_str '-' num2str(fixed_params(1)) ')./ (1 + epsi.^((']; 
        end
    end    
...
%To produce error estimates on predictions, use the optional output
%arguments R, J, CovB, or MSE as inputs to nlpredci.
%To produce error estimates on the estimated coefficients, beta,
%use the optional output arguments R, J, CovB, or MSE as inputs to nlparci.
stat.param=BETA';
...
lower=stat.ypred-delta;
upper=stat.ypred+delta;
% figure
% plot(x,y,'ko') % observed data
% hold on
% plot(x,stat.ypred,'k','LineWidth',2)
% [a,b]=min(abs(stat.ypred-xx/2))%0.0244 7457
% stat.ypred(b)%26.0151
% k(b)%7457/63457
% plot(x,[lower,upper],'b--','LineWidth',1.5)
free_param_count=0;
for i=1:4;
    if isnan(fixed_params(i))
        free_param_count=free_param_count+1;
